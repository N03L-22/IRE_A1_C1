...
\subsection{XLM-RoBERTa's collapse is anisotropy --- and it is fixable \F{F37,F71}}
...
collapse into a narrow cone. MiniLM achieves a 348$\times$ larger margin from \emph{half} the
dimensions, so it ships; XLM-R is reported as a measured failure rather than a silent substitution.

\textbf{But ``cannot separate'' was too strong a conclusion, and finding the limit of our own claim
is the more useful result \F{F71}.} Subtracting the mean direction --- one line --- takes the margin
from $+0.0018$ to $\mathbf{+0.3875}$, a \textbf{215$\times$} improvement; with 128-d truncation it
\textbf{separates cleanly} at $+0.5070$. The information was always there, drowned by one dominant
direction. Whitening, the textbook fix, \emph{fails} ($+0.0020$) --- it removes the signal with the
anisotropy.

Most tellingly: even at the collapsed baseline, \textbf{4 of 5 related pairs still rank in the top 5
by cosine}. Retrieval consumes \emph{ranking}, not absolute similarity, so the property the system
depends on was largely intact while the property the probe measured was destroyed. \textbf{A smoke
test can prove something unusable only if it tests the thing you use} --- the same lesson as the
BM25 \texttt{score\_subset} check, where verifying rank agreement rather than absolute scores was
the right call. MiniLM still wins on both axes (+0.6270 baseline, 5/5 on ranking), so the shipped
choice is unchanged; what changes is the strength of the claim. \emph{Caveat: $n=5$ pairs is a smoke
test, and no retrieval metric was run on centered vectors --- no recall improvement is claimed.}
...
\item \textbf{Parallelism was memory-bound, and the fix was representation --- twice
\F{F36,F64--F70}.} A worker peaked at 15.1\,GB, of which the index was only 1.8\,GB; the rest was
116.8M click ids as Python \texttt{str}. Storing them as \texttt{array('i')} indices gave
\textbf{9.25\,GB and 45\% faster}. Pushing the same idea further --- holding histories as contiguous
\texttt{int32} arrays read straight from parquet row groups, never as Python objects --- cut the load
from \textbf{160.8\,s to 4.6\,s} and $\sim$13\,GB to \textbf{0.93\,GB} \F{F64}.

That representation then enabled something the object version could not do. Every worker had been
rebuilding the \emph{same} immutable index, articles and histories, so $N$ workers meant $N$ copies
(19.3\,GB each, measured). Building them once in the parent and letting \texttt{fork}'s
copy-on-write share them takes the true marginal cost of a worker --- \texttt{Private\_Dirty}, not
\texttt{RSS} --- to \textbf{0.38\,GB, a 48$\times$ reduction} \F{F70}. This is only possible with
arrays: CPython refcounting \emph{writes} to an object header whenever a child touches it, dirtying
the page and forcing a private copy, whereas an array's data buffer is never touched by refcounting.
...
\midrule
2 workers \emph{(produced the submission)} & 2 & 48\,GB req.\ on 31\,GB & $\sim$49 min \\
Columnar histories \F{F64,F67} & 3 & 13\,GB & 33 min \\
\textbf{+ fork-shared state \F{F70}} & \textbf{6} & \textbf{10\,GB} & \textbf{22 min} \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Every one of these produced a byte-identical submission} (SHA-256 \texttt{066de46d\ldots},
13,336,711 lines) --- the gate the work was held to, since a performance change that alters the
output is a defect, not an optimisation. The 2-worker row is the configuration that thrashed:
$\sim$140{,}000 blocks/s of swap I/O with all 32 cores 75--100\% \emph{idle}.
...
\item \textbf{ANN: the defaults break at 10$\times$, the method does not \F{F49--F53}.} Two
corrections got us here, and both were tuning rather than limits. First, an early benchmark on
\emph{uniform-random} vectors measured HNSW losing 11--66\% of the exact answer; random vectors in
384 dimensions are near-orthogonal, the one input a proximity graph cannot exploit. On
\emph{clustered} vectors --- the structure real embeddings have --- the loss is 0.03--1.25\%.
Second, at 1M vectors a fixed \texttt{efSearch}=128 does collapse to \textbf{0.68} recall, because
HNSW explores a fixed candidate count regardless of corpus size, so the \emph{fraction} inspected
shrinks as it grows.

Sweeping both dials at 1M (exact baseline 22.8\,ms/query):

\begin{center}\footnotesize
\begin{tabular}{@{}rrrrrr@{}}
\toprule
$M$ & \texttt{ef} & recall & ms/query & vs exact & graph \\
\midrule
16 & 1024 & 0.9588 & 0.34 & 68$\times$ & 0.13 GB \\
32 & 512 & 0.9793 & 0.34 & 67$\times$ & 0.26 GB \\
\textbf{64} & \textbf{512} & \textbf{0.9947} & 0.51 & \textbf{45$\times$} & 0.52 GB \\
64 & 1024 & 0.9990 & 0.58 & 39$\times$ & 0.52 GB \\
\bottomrule
\end{tabular}
\end{center}

At \emph{fixed latency} a denser graph always wins --- more links means fewer dead ends, so the walk
reaches the same recall exploring fewer candidates. \textbf{Shipped $M{=}64$ with \texttt{ef} derived
from corpus size: 99.5\% of the exact answer, 45$\times$ faster.} Index memory is not the binding
constraint (0.52 GB of graph against 1.54 GB of vectors); build time is a 183\,s one-off.
\end{itemize}
...
null result it is \F{F28}. (iii)~MIND's test window is a single day, so one day's topical anomaly
moves every MIND number. (iv)~Reproduction is verified for both datasets by regenerating each
submission and comparing SHA-256: MIND \texttt{ba275ef0\ldots} in 2{,}282\,s against a 2{,}288\,s
baseline, EB-NeRD \texttt{066de46d\ldots}. \emph{Caveat:} EB-NeRD's check ran on the optimised path
(byte-identical, so the pipeline is deterministic), not on the exact 2-worker configuration that
produced the upload --- that configuration no longer fits this machine.

\textbf{The methodological finding, which outranks the results.} Offline and leaderboard disagreed
\textbf{three times out of three, in both directions}: $n{=}800$ \emph{understated} MIND
(0.4981 vs 0.5568) \F{F34}; fusion was \emph{not significant} offline yet gained \textbf{+0.0366}
on 2.37M impressions \F{F42}; 256-d was significant offline at $+0.0175$ and moved the leaderboard
\textbf{+0.0004} \F{F58}. The harness remains sound for what it was built for --- leakage, slicing,
the recency effect --- but \textbf{no retriever or parameter choice here is settled until it has
been submitted}, and this note labels each such claim rather than rounding it into a result.
